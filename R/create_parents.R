...
create_parents <- function(map.info, num.parents, max.delt.allele, par.markers.unique = F, heterozygous.markers=F, inbred.parents=F, QTL.sd = 0.25){
  ...
  d.marker.genotypes <- vector(); r.marker.genotypes <- vector(); marker.genotypes <- c()
  if(par.markers.unique == T){
    if(inbred.parents == T){
      ...
      d.marker.genotypes <- rep("G",length(par.IDs))
      r.marker.genotypes <- rep("g",length(par.IDs))
      }
  ...
  while(anyNA(parents)){
    if(max.delt.allele == 0 ){
      for(par in 1:num.parents){
        ...
        minor1 <- QTLSNPs[which(map.info$MAF[QTLSNPs] < maf.test)]
        maf.test <- runif(n = num.SNPQTL,min = 0,max = 1)
        minor2 <- QTLSNPs[which(map.info$MAF[QTLSNPs] < maf.test)]
        ...
        rand1<-runif(num.markers, min=0, max=.49)
        rand2<-runif(num.markers, min=0, max=.49)
        minor1<-all.markers[which(map.info$MAF[all.markers] > rand1)]
        major1 <- all.markers[which(!all.markers %in% minor1)]
        minor2<-all.markers[which(map.info$MAF[all.markers] > rand2)]
