#' Extract Selections
#'
#' This function extracts selections from progeny generated utilizing specified strategy.
#' @param map.info The number of chromosomes to simulate
#' @param cross.design The genetic map length in centi-morgans
#' @param past.tgv Number of markers that can be used for genomic selection or estimated relatedness
#' @param past.phenos The total number of QTL to place on the genetic map
#' @param parent.info The number of total.qtl which will be SNPs
#' @param progeny.info Type of distance function to use in calculating recombination frequences.  Options are: "haldane" & "kosambi".  (Default = "haldane)
#' @param progeny.phenos Default is NULL so that loci are randomly distributed to linkage groups. Options: "even" or "list"
#' @param selection.strategy Changes the assignment of markers to linkage groups from random to equally spaced ("equally-spaced")
#' @param among.family.selection A vector defining number of loci on each linkage group. Length of vector should be equal to number of linkage groups. Default is 'NULL'.
#' @param snp.qtl.maf A vector of 2 numbers indiciating the min and max range of SNP QTL MAFs
#' @param within.family.selection Default is Null. Unless specified markers MAFs will be drawn from beta distribution
#' @param num.selections.within.family The chromosome size range from the mean length of chromosomes (Default: 0.2)
#' @param num.selections.among.family The chromosome size range from the mean length of chromosomes (Default: 0.2)
#' @param relationship.matrix.type The chromosome size range from the mean length of chromosomes (Default: 0.2)
#' @param prefix The chromosome size range from the mean length of chromosomes (Default: 0.2)
#' @param rep.num The chromosome size range from the mean length of chromosomes (Default: 0.2)
#' @param reduced logical. Saves the output of genetic map (Default: FALSE)
#' @param weighted logical. Saves the output of genetic map (Default: FALSE)
#' @param num.cores logical. Saves the output of genetic map (Default: FALSE)
#' @keywords cat
#' @export
#' @examples
#'  extract_selections(save=T,num.chromos = 12,map.length = 1800,num.markers = 120,total.qtl = 120, num.snpqtl = 120,distribute.loci = "even",marker.distribution = "equally-spaced",snp.qtl.maf = c(0.01,0.02))
...
extract_selections <- function(map.info, cross.design, past.tgv,
...

  {
    ...
    if(is.null(parent.info$phenos)){
      parent.info$phenos <- parent.phenos$phenos
      parent.info$genetic.values <- parent.phenos$genetic.values
    }
    if (generation == 1) {
